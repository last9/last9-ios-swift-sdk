...
/// - Users: `Last9RUM.identify(...)` stamps `user.*` on every span
public final class Last9RUM {
    public static var shared: Last9RUM?

    private let tracerProvider: TracerProviderSdk
    private let loggerProvider: LoggerProviderSdk?
    private let logProcessor: BatchLogRecordProcessor
    ...
    private init(
        ...
        // 2. Synthetic origin — ios://com.your.bundleid (overridable for testing)
        let origin = overrideOrigin ?? "ios://\(bundleId)"
        ...
        let logProc = BatchLogRecordProcessor(
            logRecordExporter: logExporter,
            scheduleDelay: 5,
            maxQueueSize: 2048,
            maxExportBatchSize: 512
        )
        self.logProcessor = logProc
        self.loggerProvider = LoggerProviderBuilder()
            .with(resource: resource)
            .with(processors: [logProc])
            .build()
    ...
    private static func buildResource(serviceName: String, environment: String) -> Resource {
        ...
        var attrs: [String: AttributeValue] = [
            ...
            "service.version": .string(appVersion == "unknown" ? appVersion : "\(appVersion)+\(buildNumber)"),
            "deployment.environment": .string(environment),
            "service.namespace": .string("mobile"),
            "last9.rum.sdk.version": .string(last9SDKVersion),
        ]
    ...
    private static func installCrashHandler() {
        NSSetUncaughtExceptionHandler { exception in
            ...
            Last9RUM.shared?.watchdogDetector.markCrashHandlerFired()
            ...
            Last9RUM.shared?.tracerProvider.forceFlush()
            Last9RUM.shared?.logProcessor.forceFlush()
    ...
    public static func tracer(_ name: String = "app") -> Tracer {
