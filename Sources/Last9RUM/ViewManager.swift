...
final class ViewManager {
    ...
    func startView(name: String) {
        ...
        // Set as active context so all spans created during this view
        // (HTTP, custom) inherit the same trace ID automatically.
        ...
        if isFirstView {
            Last9RUM.shared?.appLaunchTracker.recordFirstViewAppeared()
            isFirstView = false
        }
    }
    ...
    func endCurrentView() {
        ...
        // Remove from active context before ending so no new spans are parented here
...
extension UIViewController {
    ...
    public var last9ViewName: String? {
